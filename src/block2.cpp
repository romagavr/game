...
void Block2::setCoords(double PosX, double PosY, double PosZ)
{
    posX = PosX;
    posY = PosY;
    posZ = PosZ;
...
void Block2::changeCoords(double delY)
{
    posY -= delY;
}

void Block2::drawStatic()
{
...
void Block2::draw(float anglex, float angley, float anglez, double PosX, double PosY, double ScValue)
{
    int modelViewProjMatrixLocation = glGetUniformLocation(shaderProgram, "uModelViewProjMat");
    int TextureLocation = glGetUniformLocation(shaderProgram,"uTexture");
